# AGENT.md — Store Apps Platform (Order Management System)
...
2. **Assigned employee**: required from the moment an order enters `IN_PROGRESS` onward (i.e. required for `IN_PROGRESS` and `COMPLETE`, not required in `OPEN`). Once assigned, a normal transition never unassigns it.
...
- **Modular by domain**: `orders`, `employees`, `customers`, each with `domain/ application/ infrastructure/` layers.
